...
request.onerror = function (event) {
  ...
  request.onupgradeneeded = function () {
    ...
    //2
    const store = db.createObjectStore("Signs", { keyPath: "gloss" });
  
    //3
    store.createIndex("access_frames", ["ins"], { unique: false });
 ...
 videoElement.autoplay = true;
  ...
  async function todosloop (){
    for (const elements of temp[0]){
      ...
      videoElement.src = `alphabets/9.mp4`;
 ...
 function todos(classes){
  ...
  async function done() {
    ...
    function hello () {
      ...
      console.log(classes,typeof array);
      store.put({ gloss: classes, ins: (array) });
      console.log("posted in index db")

      
      const idQuery = store.get(classes);
